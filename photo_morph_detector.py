...
class PhotoMorphDetector:
    ...
    def analyze_image_compression(self, image_path: str) -> Dict[str, float]:
        ...
        try:
            ...
            # Inconsistent compression suggests editing
            compression_inconsistency = min(compression_variance / (compression_mean + 1e-6), 1.0)
    ...
    def analyze_noise_patterns(self, image_path: str) -> Dict[str, float]:
        ...
        try:
            ...
            noise_inconsistency = min(noise_variance_std / (noise_variance_mean + 1e-6), 1.0)
    ...
    def analyze_edge_consistency(self, image_path: str) -> Dict[str, float]:
        ...
        try:
            ...
            # Calculate edge inconsistency
            edge_density_std = np.std(edge_densities)
            edge_density_mean = np.mean(edge_densities)
            edge_inconsistency = min(edge_density_std / (edge_density_mean + 1e-6), 1.0)
    ...
    def calculate_morph_score(self, analyses: Dict[str, Dict[str, float]]) -> Tuple[float, str, Dict[str, float]]:
        """Calculate overall morph probability from all analyses."""
        try:
            # Extract key indicators
            compression_score = analyses['compression']['compression_inconsistency']
            noise_score = analyses['noise']['noise_inconsistency']
            edge_score = analyses['edge']['edge_inconsistency']
            lighting_score = analyses['lighting']['lighting_inconsistency']
            color_score = analyses['color']['color_inconsistency']
            texture_score = analyses['texture']['texture_inconsistency']
            ...
            morph_probability = min(max(morph_probability, 0.0), 1.0)

            # Determine classification
            if morph_probability < 0.3:
                classification = "Real Photo"
                confidence = 1.0 - morph_probability
            elif morph_probability < 0.7:
                classification = "Possibly Edited"
                confidence = 0.5
            else:
                classification = "Likely Morphed/Edited"
                confidence = morph_probability
